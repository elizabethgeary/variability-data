...
regression.dat <- data.frame(sub = s1.dat.wide$sub,
                             ipsi_rightput = nu.data$scores[,'Factor1'],
                            Dorsal_acc_contraput= nu.data$scores[,'Factor2'],
                             ACC_caud_contracaud = nu.data$scores[,'Factor3'])

# now, show the data
regression.dat %>% head(87)

write.csv(regression.dat, paste(file = "facsol.csv", sep="/"), row.names=  FALSE)
